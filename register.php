...
$uniones = [];
$res = $mysqli->query("SELECT id, nombre FROM union_iasd ORDER BY nombre");
    ...
    if (empty($name) || empty($email) || empty($password)) {
        ...
        if ($check->num_rows > 0) {
            ...
            if (!$inst) {
                ...
                // Insertar usuario
                $stmt = $mysqli->prepare(
                    "INSERT INTO users (name, email, password, roles, union_id, institucion_id)
                    VALUES (?, ?, ?, ?, ?, ?)"
                );
